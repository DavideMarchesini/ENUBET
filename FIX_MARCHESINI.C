...
void FIX_MARCHESINI(){
  ...
  - A defines the number of sigma for the t_0 as A*sigma 
  */
  string p="pedestal_data.txt";
  string filename="Run15_list.root";
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    for(const auto &v : map){
      ...
      if(mRR[fersIndex]==0 || mRR[fersIndex]==1){
        CONC=A;} 
      else{
        CONC=1;} 
      if (dEeq>CONC*thresholdLG.at(fersIndex))
